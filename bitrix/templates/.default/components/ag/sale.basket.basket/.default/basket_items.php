						...
						foreach ($arResult["GRID"]["HEADERS"] as $id => $arHeader):
							...
							?>
								...
								<td class="item">
									...
									?>
									<div class="ag-basket-store-exists-title">Наличие в центрах выдачи</div>
									<table class="ag-basket-store-exists-table">
                                        <tr><th>Центр выдачи</th><th>наличие(шт.)</th></tr>
									<?
                                        $res = CCatalogStoreProduct::GetList(array(),array("=PRODUCT_ID"=>$arItem["PRODUCT_ID"],"!STORE_ID"=>1));
                                        while($store = $res->GetNext()){
                                            ?>
                                            <tr>
                                                <td class="mfc"><a href="<? echo $store["STORE_DESCR"]?>"><? echo  $store["STORE_NAME"]?></a></td>
                                                <td class="amount"><?= $store["AMOUNT"]?></td>
                                            </tr>
                                            <?
                                        }
									?>
