...
if (is_object($cp))
{
    ...
    if(preg_match("#^([\d\s]+).+$#",$arResult["PRICE_FORMATED"],$m)){
        $arResult["PRICE_FORMATED"] = $m[1]." ".get_points($m[1]);
    }
    if(preg_match("#^([\d\s]+).+$#",$arResult["PRODUCT_SUM_FORMATTED"],$m)){
        $arResult["PRODUCT_SUM_FORMATTED"] = $m[1]." ".get_points($m[1]);
    }
    if(preg_match("#^([\d\s]+).+$#",$arResult["PRICE_DELIVERY_FORMATED"],$m)){
        $arResult["PRICE_DELIVERY_FORMATED"] = $m[1]." ".get_points($m[1]);
    }
    
    foreach($arResult["BASKET"] as $key=>$prod){
        if(preg_match("#^([\d\s]+).+$#",$prod["PRICE_FORMATED"],$m)){
            $arResult["BASKET"][$key]["PRICE_FORMATED"] = $m[1]." ".get_points($m[1]);
        }
        if(preg_match("#^([\d\s]+).+$#",$prod["DISCOUNT_PRICE_PERCENT_FORMATED"],$m)){
