<div class="container-fluide">
  <div class="row">
    <div class="col-12">
      <div class="warning-border">
        <p><i class="fas fa-exclamation-triangle"></i>Для заказа данного поощрения необходимо набрать 800 баллов</p>
      </div>
    </div>



      <div class="col-3 col-xl-1">
        <!--@Бляхи@-->
        <div class="product-label">
            <div class="label-stock">
              Акция
            </div>
            <div class="label-new">
              Новинки
            </div>
            <div class="label-hot">
              Хит
            </div>
        </div>

        <!--@Боковой слайдер@-->
        <div class="product-slider">
          <i class="fas fa-angle-up"></i>
          <div class="product-slider-img"></div>
          <div class="product-slider-img"></div>
          <div class="product-slider-img"></div>
          <div class="product-slider-img"></div>
          <i class="fas fa-angle-down"></i>
        </div>

      </div>


      <div class="col-9 col-xl-7">
        <div class="product-img">
          <img id="center-block" class="center-block" src="/local/components/ag/card/templates/desktop2018/image/big_theatre_800x600.jpg" alt="product-img">
          <div class="product-heart">
            <i class="far fa-heart"></i>
            <p id="p-heart">1634</p>
          </div>

        </div>
      </div>

      <!--@ Описание товара @-->
      <div class="col-12 col-sm-12 col-md-12 col-lg-12 col-xl-4">
        <div class="product-description">
            <div class="product-description-header">
                <h2>Чехол для IPhone 4/4S с символикой проекта</h2>
                <div class="row">
                  <div class="col-auto">
                      <p id="product-date">Использовать до: 27.05.2018</p>
                  </div>
                  <div class="col-auto mg-left">
                      <p id="attribute"><abbr title="attribute">12 отзывов</abbr></p>
                  </div>
                  <div class="col-auto mg-left">
                    <p><i class="fas fa-star"></i></p>
                    <p><i class="fas fa-star"></i></p>
                    <p><i class="fas fa-star"></i></p>
                    <p><i class="fas fa-star"></i></p>
                    <p><i class="far fa-star"></i></p>
                  </div>
                </div>
            </div>
            <div class="product-description-main">
              <div class="col">
                <div class="row">
                  <div class="col-2">
                    <p class="product-attr">Статус:</p>
                  </div>
                  <div class="col-4">
                    <span>В наличии</span>
                  </div>
                </div>
                <div class="row">
                  <div class="col-2">
                  <p class="product-attr">Стоимость:</p>
                  </div>
                  <div class="col-4">
                  <span>800 балов</span>
                  </div>
                </div>
                  <div class="row">
                    <div class="col-2">
                        <p class="product-attr">Количество:</p>
                    </div>
                    <div class="col-4">
                        <span>-1+</span>
                    </div>
                  </div>
                  <div class="row">
                    <div class="col-4">
                        <p class="product-attr">Дата и время:</p>
                    </div>
                    <div class="col-8">
                      <div class="product-btn-wrap">
                        <button type="button" class="ag-btn-date btn btn-outline-success">29 августа (ср)</button>
                        <button type="button" class="ag-btn-date btn btn-outline-success">01 сентября (сб)</button>
                        <button type="button" class="ag-btn-date btn btn-outline-success">02 сентября (вс)</button>
                        </div>
                      </div>
                    </div>
                    <div class="row">
                      <div class="col-2">
                          <p class="product-attr">Категория:</p>
                      </div>
                      <div class="col-8">
                        <div class="product-description-size">
                          <button class="border br-size btn-outline-info">1</button>
                          <button class="border br-size">2</button>
                          <button class="border br-size">Партер</button>
                          <button class="border br-size">VIP lounge</button>
                        </div>
                      </div>
                    </div>
                  </div>

              </div>
                <div class="col-auto">
                  <div class="row">
                    <div class="col-4">
                        <p class="product-attr">Где получить:</p>
                    </div>
                    <div class="col-6">
                      <div class="btn-group">
                        <label>
                           <input class="radio" type="radio" name="radio-test" checked>
                           <span class="radio-custom"></span>
                           <span class="label">На электронную почту</span>
                         </label>
                        </div>

                    </div>
                  </div>
              </div>

              <div class="col-auto">
                <div class="row">
                  <div class="col-3">
                      <p class="product-attr">На складе:</p>
                  </div>
                  <div class="col-2" style="padding-top: 1rem">
                    <span>Мало</span>
                  </div>
                </div>
              </div>

              <div class="product-description-btn">
                <div class="row">
                  <div class="col-7">
                    <button type="button" class="btn btn-primary btn-lg">Заказать за 800 баллов</button>
                  </div>
                  <div class="col-2 col-sm-auto">
                      <a href="#"><i id="product-heart" class="far fa-heart"></i></a>
                  </div>
                  <div class="col-2 col-sm-1">
                    <a href="#"><i id="product-share" class="fas fa-share-alt"></i></a>
                  </div>
                </div>
              </div>
            </div>
        </div>
      </div>

  </div>


<!--@Аккордеон@-->
<main class="ag-accordion">
  <div class="container-fluide">
  <div class="row">
    <div class="col-12">
      <div class="product-accordion">
        <div class="accordion" id="accordionExample">
        <div class="card">
          <div class="card-header" id="headingOne">
            <h5 class="mb-0">
              <button class="btn btn-link" type="button" data-toggle="collapse" data-target="#collapseOne" aria-expanded="true" aria-controls="collapseOne">
                Описание товара
              </button>
            </h5>
          </div>
          <div id="collapseOne" class="collapse show" aria-labelledby="headingOne" data-parent="#accordionExample">
            <div class="card-body">
              Силиконовый чехол для IPhone 4/4s с символикой проекта "Активный гражданин". Детали:отверстия для камеры, зарядного устройства и кнопки регулировки звука.
              <b>Внимание: срок действия вашего заказа 14 дней с момента оформления.</b>
            </div>
          </div>
        </div>
        <div class="card">
          <div class="card-header" id="headingTwo">
            <h5 class="mb-0">
              <button class="btn btn-link collapsed" type="button" data-toggle="collapse" data-target="#collapseTwo" aria-expanded="false" aria-controls="collapseTwo">
                Правила получения
              </button>
            </h5>
          </div>
          <div id="collapseTwo" class="collapse" aria-labelledby="headingTwo" data-parent="#accordionExample">
            <div class="card-body">
              Силиконовый чехол для IPhone 4/4s с символикой проекта "Активный гражданин". Детали:отверстия для камеры, зарядного устройства и кнопки регулировки звука.
              <b>Внимание: срок действия вашего заказа 14 дней с момента оформления.</b>
            </div>
          </div>
        </div>
        <div class="card">
          <div class="card-header" id="headingThree">
            <h5 class="mb-0">
              <button class="btn btn-link collapsed" type="button" data-toggle="collapse" data-target="#collapseThree" aria-expanded="false" aria-controls="collapseThree">
                Правила отмены
              </button>
            </h5>
          </div>
          <div id="collapseThree" class="collapse" aria-labelledby="headingThree" data-parent="#accordionExample">
            <div class="card-body">
              Силиконовый чехол для IPhone 4/4s с символикой проекта "Активный гражданин". Детали:отверстия для камеры, зарядного устройства и кнопки регулировки звука.
              <b>Внимание: срок действия вашего заказа 14 дней с момента оформления.</b>
            </div>
          </div>
        </div>
        <div class="card">
          <div class="card-header" id="headingThree">
            <h5 class="mb-0">
              <button class="btn btn-link collapsed" type="button" data-toggle="collapse" data-target="#collapseFor" aria-expanded="false" aria-controls="collapseThree">
                Отзывы
              </button>
            </h5>
          </div>
          <div id="collapseFor" class="collapse" aria-labelledby="headingThree" data-parent="#accordionExample">
            <div class="card-body">
              Силиконовый чехол для IPhone 4/4s с символикой проекта "Активный гражданин". Детали:отверстия для камеры, зарядного устройства и кнопки регулировки звука.
              <b>Внимание: срок действия вашего заказа 14 дней с момента оформления.</b>
            </div>
          </div>
        </div>
        <div class="card">
          <div class="card-header" id="headingThree">
            <h5 class="mb-0">
              <button class="btn btn-link collapsed" type="button" data-toggle="collapse" data-target="#collapseFive" aria-expanded="false" aria-controls="collapseThree">
                Вопросы и ответы
              </button>
            </h5>
          </div>
          <div id="collapseFive" class="collapse" aria-labelledby="headingThree" data-parent="#accordionExample">
            <div class="card-body">
              Силиконовый чехол для IPhone 4/4s с символикой проекта "Активный гражданин". Детали:отверстия для камеры, зарядного устройства и кнопки регулировки звука.
              <b>Внимание: срок действия вашего заказа 14 дней с момента оформления.</b>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  </div>
  </div>
</main>

<article class="product-recommended">
  <div class="container-fluide">
    <h2>Рекомендуемые товары</h2>
      <div class="row">
        <div class="col-12 col-sm-6 col-xl-2">
          <div class="product-main">
            <div class="product-header">
              <h3>3499</h3>
              <p class="product-recommended-ball">Баллов</p>
              <p class="product-recommended-heart"><i class="far fa-heart"></i>1674</p>
            </div>
              <img class="product-recommended-img" src="/local/components/ag/card/templates/desktop2018/image/recomend-product.png" alt="recommend-product">
              <p class="product-recommended-item">Рюкзак зеленый</p>
          </div>
        </div>
        <div class="col-12 col-sm-6 col-xl-2">
          <div class="product-main">
            <div class="product-header">
              <h3>3499</h3>
              <p class="product-recommended-ball">Баллов</p>
              <p class="product-recommended-heart"><i class="far fa-heart"></i>1674</p>
            </div>
              <img class="product-recommended-img" src="/local/components/ag/card/templates/desktop2018/image/recomend-product.png" alt="recommend-product">
              <p class="product-recommended-item">Рюкзак зеленый</p>
          </div>
        </div>
        <div class="col-12 col-sm-6 col-xl-2">
          <div class="product-main">
            <div class="product-header">
              <h3>3499</h3>
              <p class="product-recommended-ball">Баллов</p>
              <p class="product-recommended-heart"><i class="far fa-heart"></i>1674</p>
            </div>
              <img class="product-recommended-img" src="/local/components/ag/card/templates/desktop2018/image/recomend-product.png" alt="recommend-product">
              <p class="product-recommended-item">Рюкзак зеленый</p>
          </div>
        </div>
        <div class="col-12 col-sm-6 col-xl-2">
          <div class="product-main">
            <div class="product-header">
              <h3>3499</h3>
              <p class="product-recommended-ball">Баллов</p>
              <p class="product-recommended-heart"><i class="far fa-heart"></i>1674</p>
            </div>
              <img class="product-recommended-img" src="/local/components/ag/card/templates/desktop2018/image/recomend-product.png" alt="recommend-product">
              <p class="product-recommended-item">Рюкзак зеленый</p>
          </div>
        </div>
        <div class="col-12 col-sm-6 col-xl-2">
          <div class="product-main">
            <div class="product-header">
              <h3>3499</h3>
              <p class="product-recommended-ball">Баллов</p>
              <p class="product-recommended-heart"><i class="far fa-heart"></i>1674</p>
            </div>
              <img class="product-recommended-img" src="/local/components/ag/card/templates/desktop2018/image/recomend-product.png" alt="recommend-product">
              <p class="product-recommended-item">Рюкзак зеленый</p>
          </div>
        </div>
        <div class="col-12 col-sm-6 col-xl-2">
          <div class="product-main">
            <div class="product-header">
              <h3>3499</h3>
              <p class="product-recommended-ball">Баллов</p>
              <p class="product-recommended-heart"><i class="far fa-heart"></i>1674</p>
            </div>
              <img class="product-recommended-img" src="/local/components/ag/card/templates/desktop2018/image/recomend-product.png" alt="recommend-product">
              <p class="product-recommended-item">Рюкзак зеленый</p>
          </div>
        </div>
      </div>
    </div>
</article>
