...
$(function() {
  ...
  function checkProductsWidth() {
    var catalogItemsWrapper = $('.desktop-products-container');
    var productsWidth = $('.desktop-products-container').width();
    console.log(productsWidth);

    if (productsWidth < 700) {
      $(catalogItemsWrapper).addClass('cutProductInfo');
    }
    else {
      $(catalogItemsWrapper).removeClass('cutProductInfo');
    }
  };
  ...
  function setProductHeight() {
    var productWidth = $(".desktop-product").width();
    productWidth = productWidth - 4;
    $(".desktop-product-inner").css("height", productWidth);
  };

  // setProductHeight();
  // Кнопка выдвигающегося фильтра

  $('#catalogHeaderFilter').on('click',function () {

    var catalogWrapper = $('#productsWrapper');
    var catalogItemsWrapper = $('.desktop-products-container');

    if ($(catalogWrapper).hasClass('hide-filter')) {
      $(catalogWrapper).removeClass('hide-filter');
      // $(catalogItemsWrapper).addClass('filterOpened');
    }
    else {
      $(catalogWrapper).addClass('hide-filter');
      // $(catalogItemsWrapper).removeClass('filterOpened');
    }

    function respondItems() {
       checkProductsWidth();
    ...
    setTimeout(respondItems, 1000);

  });

  // ======

  if ( document.querySelector('#desktopCatalogFilterForm') != null ) {

    $('#desktopCatalogFilterForm').on('click', '.desktop-products-filter-item__header' ,function () {

        // Set active for clicked item
        $(this).toggleClass("active");
        var accordionCurrentContent = $(this).next(".desktop-products-filter-item__content");
        accordionCurrentContent.toggleClass("active");
        accordionCurrentContent.slideToggle(400);
  ...
  $('.desktop-products-filter').stickybits({
    stickyBitStickyOffset: 65,
    useStickyClasses: true,
  });

  $('.desktop-catalog-header').stickybits({
  ...
  $('.desktop-catalog-sort__btn').on('click',function () {

    if ( $(this).hasClass("active")) {
      // console.log("Дропдаун был открыт");
      $(this).removeClass("active");
      $(this).next().removeClass("show-dropdown");
    }
    else {
      // console.log("Дропдаун был закрыт");

      $('.desktop-catalog-sort__btn').removeClass("active");
      $('.desktop-catalog-sort__dropdown').removeClass("show-dropdown");
  ...
  // Ставим правильную текстовку после выбора сортировки

  $('.list-dropdown__link').on('click',function () {
    var linkTitle = $(this).attr("data-dropdownName");
    var linkGrid = $(this).attr("data-gridStyle");
    var linkContainer = $(this).parent();
    var dropdownWrapper = $(this).parents(".dropdown-wrapper");
    var dropdownBtn = $(dropdownWrapper).find(".desktop-catalog-sort__btn");
    var dropdownContainer = $(dropdownWrapper).find(".desktop-catalog-sort__dropdown");
    var linkDestination = $(dropdownWrapper).find(".desktop-catalog-sort__btn-current");

    $(dropdownContainer).find(".list-dropdown__item").removeClass("selected");
    $(dropdownBtn).removeClass("active");
    $(dropdownContainer).removeClass("show-dropdown");
    $(linkContainer).addClass("selected");
    $(linkDestination).html(linkTitle);
    ...
    if ( linkGrid === 'gridBig'  ) {
      ...
      setTimeout(respondItems, 400);
    }

    if ( linkGrid === 'gridSmall' ) {
      $(".desktop-products-container").removeClass("desktop-products-container--gridList");
  ...
  // Функция для установки Выбора всех

  // ===================
  // Remove or Set OptionAllCheckbox
  // ===================

  $('.desktop-checkbox__label').on('click',function (e) {
    var clickedCheckbox = $(this).prev();
    var dropdownContainer = $(this).parents(".desktop-products-filter-item__content");
    var dropdownContainerChildrens = dropdownContainer.children();
    var selectAllCheckbox = $(dropdownContainer).find(".filterCheckboxAll");

    if (clickedCheckbox.hasClass("filterCheckboxAll")) {
      e.preventDefault();
  ...
  function checkFormState() {
    ...
    if ($('#desktopCatalogFilterForm').hasClass('form-changed')) {
      // form is already changed
    }
    else {
      $('#desktopCatalogFilterForm').addClass('form-changed');
      $('#desktopProductsFilterSubmit').prop("disabled", false);
      $('#desktopProductsFilterReset').prop("disabled", false);
    }
  };
  ...
  $('#desktopCatalogFilterForm input').on('change',function (e) {
    checkFormState();
  });
  ...
  var numInput = document.querySelector('.default-desktop-input--price');
  ...
  var numInputMax = document.querySelector('.default-desktop-input--priceMax ');

  // Listen for input event on numInput.
  if(numInput)
  numInputMax.addEventListener('input', function(){
      // Let's match only digits.
      var num = this.value.match(/^\d+$/);
      if (num === null) {
          // If we have no match, value will be empty.
  ...
  // how it work = https://stackoverflow.com/questions/31575496/prevent-negative-inputs-in-form-input-type-number

  // ===================
  // Reset Form Button
  // ===================

  $('#desktopProductsFilterReset').on('click',function (e) {
    // Prevent Default Reset
    e.preventDefault();
    document.location.hash='';
    resetDesktopCheckboxState();
    $('#desktopCatalogFilterForm').removeClass('form-changed');
    $('#desktopProductsFilterSubmit').prop("disabled", true);
    $('#desktopProductsFilterReset').prop("disabled", true);
  });
