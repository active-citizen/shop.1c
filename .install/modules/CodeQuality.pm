...
=cut 
    sub pDependReport{
        ...
        $report .=  "Хорошим показателем является значение не более 3. Если это не выполняется, то, скорее всего, необходимо некоторые блоки кода выделить в отдельные функции. Далее выводятся только классы, для которых не выполняется\n\n";
        my $subreport = '';
        foreach my $class(@{$self->{classes}}){
            $subreport .= $self->metricaLine($class->{file}->{name}."::".$class->{name}."::".$_->{name},$_->{ccn},0,3,1) foreach @{$class->{method}};
        }
