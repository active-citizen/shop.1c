...
class Troyka extends CIntegration
{
    ...
    function __construct($sNum = '', $sPhone = '')
    {
        ...
        if (!$sNum) {
            $sNum = $this->settings["TROYKA_CARD"]["VALUE"];
        }
        if (!$sPhone) {
            $sPhone = $this->settings["TROYKA_PHONE"]["VALUE"];
        }

        if (!$this->checkTroyka($sNum)) {
            return false;
        }
    ...
    {
        if (!$this->checkOrderNum($sOrderNum)) {
            return false;
        }
