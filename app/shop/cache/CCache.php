...
class CCache
{
    ...
    function __construct($sGroup, $sKey, $nExpires = '')
    {
        ...
        if (!$this->objMemcached->addServer($sHost, $nPort)) {
            return false;
        }
    ...
    function set($sValue)
    {
        ...
        if ($this->objMemcached && $objData = $this->__memcachedSet($sValue)) {
            return true;
        }
     ...
     * @param bool $bOnlyGroup Очищать толко заданную в конструкторе группу
     * @return bool
     */
    function clearAll($bOnlyGroup = true)
    {
