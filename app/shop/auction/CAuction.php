...
namespace app\shop\auction;

use app\shop\CAGShop;

class CAuction extends CAGShop
{
     ...
     * @return bool | int ID предложения (можно использовать в функции
     * $this-makeResult)
     */
...
//        if(!$nUserId)$nUserId = \CUser::GetID();
        ...
        if (!isset($arCloseBets)) {
            return false;
        }

        // Результаты ещё не оформлены
        if ($arCloseBets["CLOSE_COUNT"] < $arAuction["BETS_COUNT"]) {
            return false;
        }
    ...
    {
        if (!$nId) {
            return $this->addError("Не указан ID ставки");
        }
        $CDB = new \DB\CDB;
        $sQuery = "
            SELECT
