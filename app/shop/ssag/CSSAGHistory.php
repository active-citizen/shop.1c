...
class CSSAGHistory extends CSSAG
{
    ...
    function get(
        ...
        if ($nPage) {
            $arRequest["page"] = $nPage;
        }
        if ($nOnPage) {
            $arRequest["onpage"] = $nOnPage;
        }
        if (!is_null($bDebit)) {
            $arRequest["debit"] = $bDebit;
        }
        ...
        if (!isset($arAnswer["result"]["history"])) {
            return
                $this->addError("Отсутствует история начислений/списаний");
        }
        if (!isset($arAnswer["result"]["pagination"])) {
            return
