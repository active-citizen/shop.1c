...
class CCatalogProduct extends CAGShop
{
...
//            new \XPrint($arOptions,1);
        ...
        $arFilter = [];
        if (!isset($arOptions["sorting"])) {
            $arOptions["sorting"] = [];
        }
        if (!isset($arOptions["sorting"]["param"])) {
            $arOptions["sorting"]["param"] = 'price';
        }
        if (!isset($arOptions["sorting"]["direction"])) {
            $arOptions["direction"]["param"] = 'desc';
        }
...
//            fclose($fd);
        ...
        if ($arFilter["interest"]) {
            $sQuery = "
                    ...
                    WHERE
                        ...
                        -- " . ($arSectionCond ? " AND `IBLOCK_ELEMENT_ID` IN(" . implode(",",
                        $arSectionCond) . ")" : "") . "
        ...
        */
        ...
        foreach ($arMinArray as $nId) {
            ...
            foreach ($arIntersect as $nKye => $arVal) {
                if (in_array($nId, $arVal)) {
                    $nCount++;
                }
            }
            if ($nCount >= $nIntersectSetscount) {
                $arIds[] = $nId;
            }
        }
        ...
        $res = \CIBlockElement::GetList($arSorting, [
            ...
            "PREVIEW_TEXT"//,"PROPERTY_WANTS.NAME"
        ]);
        $arItems = [];
    ...
    function wish($nProductId, $sAct, $nUserId)
    {
        ...
        if (!$nProductId) {
            return $this->addError("Не указан ID товара");
        }

        if (!$nUserId) {
            return $this->addError("Не указан ID пользователя");
        }
        ...
        elseif ($sAct == 'off') {
            $iblockObj->Delete($arElement["ID"]);
        } // Сообщить об ошибке
        else {
            return $this->addError($iblock->LAST_ERROR);
        }
