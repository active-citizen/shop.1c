 ...
 * Документы в поиске
 */
class CSearchDocument extends CAGShop
{
    ...
    function parse($sText)
    {
        ...
        foreach ($arWords as $sWord) {
            ...
            if (in_array($sWord, $this->arStopWords)) {
                continue;
            }
            if (mb_strlen($sWord) < $this->nMinWordLength) {
                continue;
            }
            if (mb_strlen($sWord) > $this->nMaxWordLength) {
                continue;
            }
            $nPosition++;
